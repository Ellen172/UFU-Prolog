...
home(_Pedido) :-
    reply_html_page(
        [ title('Bootstrap 5 basico')],
        [ \pagina ]).

pagina -->
    html([ \html_post(head, % define a parte do head
                      [ meta([name(viewport),
                              content('width=device-width, initial-scale=1')])]),
           \html_root_attribute(lang,'pt-br'), % define como interpretar o documento, de acordo com o idioma
